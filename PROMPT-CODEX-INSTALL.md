...
7. 同时安装仓库中的 `windbg-analysis` Skill。以 `https://raw.githubusercontent.com/Williamhao-Y/mcp-windbg/main/.codex/skills/windbg-analysis/` 为源地址，将 `SKILL.md`、`agents/openai.yaml`、`references/analysis-modes.md` 和 `references/report-spec.md` 下载到当前项目的 `.codex/skills/windbg-analysis/`，保持子目录结构不变。若目标目录已经存在，先比较内容，只更新这四个文件，不删除未知文件，并在结果中说明。
8. 安装后验证：私有 Python 能导入 `ssl` 和 `mcp_windbg`，并确认 `python -m mcp_windbg --help` 能正常执行；确认 `.codex/config.toml` 的 `args` 中包含 `--filter-script` 和当前目录 `redact.py` 的绝对路径；确认 `.codex/skills/windbg-analysis/` 中的上述四个文件全部存在。依赖版本必须遵循当前 `mcp-windbg` 包的项目元数据，不要擅自跨主版本降级；若依赖解析冲突，停止并报告具体冲突。
